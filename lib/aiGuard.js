...
function shouldUseRedisGuard() {
  return !isLocalRuntime() && Boolean(getRedisConfig())
}
...
function isUserMessageTooLong(body) {
  ...
  const candidates = []
  if (typeof body.userMessage === 'string') candidates.push(body.userMessage)
...
async function acquireUserLock(userKey, routeName, lockToken) {
  if (shouldUseRedisGuard()) {
    ...
    logFallback(routeName, 'local_runtime_memory_lock')
  } else {
...
async function checkUserRateLimit(userKey, routeName) {
  if (shouldUseRedisGuard()) {
    ...
    logFallback(routeName, 'local_runtime_memory_rate_limit')
  } else {
    logFallback(routeName, 'redis_not_configured')
  }
