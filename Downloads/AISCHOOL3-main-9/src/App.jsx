    ...
    const handleTeacherClick = () => {
        if (user) {
            ...
            if (!role) {
                ...
                navigate('/teacher', { state: { intendedRole: 'teacher' } })
            ...
            navigate('/sign-in', { state: { intendedRole: 'teacher' } })
        }
    }
    ...
    const handleStudentClick = () => {
        if (user) {
            ...
            if (!role) {
                ...
                navigate('/student', { state: { intendedRole: 'student' } })
            ...
            navigate('/sign-in', { state: { intendedRole: 'student' } })
        }
    }
...
function ProtectedRoute({ children, requireRole }) {
    ...
    React.useEffect(() => {
        const autoAssignRole = async () => {
            ...
            // Auto-assign role if user doesn't have one and we know their intended role
            ...
            // Check access permissions if role is set
            if (role) {
        ...
        autoAssignRole()
    }, [isLoaded, user, requireRole, navigate, location, isAssigningRole])

    if (!isLoaded || isAssigningRole) {
        return (
            <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#6B7280' }}>{isAssigningRole ? 'Setting up your account...' : 'Loading...'}</p>
                </div>
            </div>
        )
    }
    ...
    if (!user) {
        return <Navigate to="/sign-in" state={{ intendedRole: requireRole }} replace />
    }

    const role = getUserRole(user)
    if (!role) {
        // Fallback to role selector if no intended role was provided
        return <RoleSelector />
    }
