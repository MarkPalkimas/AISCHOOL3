...
export default function App() {
  ...
  const goStudent = () => {
    if (isSignedIn) {
      // Teachers/Admins CAN access student view
      navigate('/student')
    } else {
      redirectToSignIn({ redirectUrl: abs('student'), signUpUrl: abs('student') })
    }
    ...
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ padding: '96px 0 64px' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <p className="hero-subtitle" style={{ margin: '0 auto 28px', maxWidth: 640 }}>
            Teachers upload materials. Students get instant, guided help — explanations, steps, and math — without
            giving away the answers.
          </p>

          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
            <button className="btn-primary" onClick={goTeacher}>
              For Teachers
            </button>
            <button className="btn-secondary" onClick={goStudent}>
              For Students
            </button>
          </div>
      ...
      <footer style={{ marginTop: 'auto', borderTop: '1px solid #E5E7EB', padding: '20px 0' }}>
        ...
        >
          <img
            ...
            style={{ width: 20, height: 20, objectFit: 'contain', opacity: 0.85 }}
          />
          <div style={{ fontSize: 12, color: '#6B7280' }}>© {new Date().getFullYear()} StudyGuideAI</div>
        </div>
      </footer>
    </div>
  )
